      ...
      <div className="flex">
        ...
        >
              ...
              <div className="flex flex-col">
                ...
                {/* Sources Section - Only show for bot messages */}
                {!isMe && message.sources && (() => {
                  let parsedSources: string[] = [];
                  try {
                    parsedSources = JSON.parse(message.sources);
                  } catch {
                    parsedSources = message.sources.split(',').map(s => s.trim()).filter(Boolean);
                  }
                  ...
                  return (
                    <div className="mt-3 space-y-1.5 max-w-md">
                      <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider px-0.5">
                        Sources
                      </p>
                      <div className="space-y-1">
                        {parsedSources.map((source, index) => (
                          <div
                            key={index}
                            className="group flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-black/[0.03] dark:hover:bg-white/[0.04] transition-colors cursor-pointer"
                          >
                            <svg className="h-3 w-3 text-gray-400 dark:text-gray-500 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <span className="text-[12px] font-medium text-gray-600 dark:text-gray-300 truncate flex-1 group-hover:text-gray-900 dark:group-hover:text-gray-100 transition-colors">
                              {source}
                            </span>
                          </div>
                        ))}
